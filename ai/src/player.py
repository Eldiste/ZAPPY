...
class Player:
    ...
    def check_response(self, resp):
        ...
        while i < len(responses):
            # if self.non_asked_elevation is True and "ko" in responses[i]:
            #     self.non_asked_elevation = False
            #     i = 0
            #     continue
            if "message" in responses[i]:
                # print("message received at pos : " + str(i) + " : " + responses[i])
                # print("len of responses: " + str(len(responses)))
                if i == len(responses) - 1:
                    self.previous_messages.insert(0, responses[i])
                    responses = self.client.receive().split("\n")[:-1]
                    # print("continuing with: " + str(responses))
                    i = 0
                    continue
                else:
                    self.previous_messages.insert(0, responses[i])
            ...
            if "Current level:" in responses[i]:
                # print("HERE IS THE LEVEL" + responses[i])
            ...
            if "dead" in responses:
                ...
                exit(0)
            # print("returning: " + responses[i] + "at pos: " + str(i) + " out of " + str(len_resp))
            return responses[i] + "\n"
    ...
    def join_call(self):
        ...
        for message in self.previous_messages:
            ...
            if int(split_message[1]) == 0:
                ...
                if int(message[1]) > highest_level:
                    highest_level = int(message[1])
                print("highest level: " + str(highest_level))
                i += 1

        message = self.previous_messages[i].split(", ")
        print("joining incantation at direction: " + message[0].split(" ")[1])
        self.move_to_direction(int(message[0].split(" ")[1]))
    ...
    def look_handler(self, view):
        if self.inventory["food"] < (MINIMAL_FOOD[self.level] + 10) and self.find_resource("food") != -1:
            ...
            # print("found food at: " + str(food_position))
    ...
    def pass_incantation(self):
        # print("nb player at tile: " + str(self.nb_player_at_tile))
        self.inventory_call()
        if self.inventory["food"] < MINIMAL_FOOD[self.level]:
            self.waiting = False
            return
        ...
        for resource in LEVELS[self.level]:
            if resource not in self.inventory:
                continue
        if self.inventory["food"] > (MINIMAL_FOOD[self.level] + 10):
        ...
        #     for resource, quantity in LEVELS[self.level].items():
        #         for _ in range(quantity):
        #             if self.inventory["food"] > (MINIMAL_FOOD[self.level] * 2):
        #                 self.set_call(resource)
        #     self.incantation_call()
        #     self.incantation_call()
    ...
    def fork_handler(self):
        ...
        # print("nbr connected: " + str(self.nbr_connected))
